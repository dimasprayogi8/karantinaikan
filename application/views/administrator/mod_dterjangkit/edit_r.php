...
<?php 
    ...
    <table class="table" width="100%" border="0">
                    <tr>
                      ...
                      <td><input type="text" name="b" id="namaDT" value="<?=$rows['namaDT']?>" class="form-control" required />
                      <input type="hidden" name="a" id="kodeDT" value="<?=$rows['kodeDT']?>" class="form-control" required /></td>
                    </tr>
                    <tr>
  ...
  <tr>
                        <td>Status Terjangkit</td>
                        <td><select name="d" id="status_terjangkit" class="form-control" required>
                            <?php
                                $dt = $this->db->query("select * from terjangkit where kodeTerjangkit ='$rows[st]'");
                            $d = $dt->row_array();
                                ?>
                                            <option value="<?=$d['kodeTerjangkit']?>"><?=$d['namaTerjangkit']?></option>
                                       <?php
                             $bedasal = $this->db->query("select * from terjangkit where kodeTerjangkit != '$rows[st]'");
                            foreach($bedasal->result_array() as $bds){
                              ?>
