...
<?php 
    ...
    <table class="table" width="100%" border="0">
                    <tr>
                      ...
                      <td><input type="text" name="b" id="namaDR" value="<?=$rows['namaDKR']?>" class="form-control" required />
                      <input type="hidden" name="a" id="kodeDR" value="<?=$rows['kodeDKR']?>" class="form-control" required /></td>
                    </tr>
                    <tr>
  ...
  <tr>
                        <td>Tingkat Kategori Resiko</td>
                        <td><select name="d" id="status_registrasi" class="form-control" required>
                            <?php
                                $dt = $this->db->query("select * from tingkatresiko where kodeTR ='$rows[tkr]'");
                            $d = $dt->row_array();
                                ?>
                                            <option value="<?=$d['kodeTR']?>"><?=$d['namaTR']?></option>
                                       <?php
                             $bedasal = $this->db->query("select * from tingkatresiko where kodeTR != '$rows[tkr]'");
                            foreach($bedasal->result_array() as $bds){
                              ?>
