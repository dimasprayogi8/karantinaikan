...
<?php 
    ...
    <table class="table" width="100%" border="0">
                    <tr>
                      ...
                      <td><input type="text" name="b" id="namaDR" value="<?=$rows['namaDR']?>" class="form-control" required />
                      <input type="hidden" name="a" id="kodeDR" value="<?=$rows['kodeDR']?>" class="form-control" required /></td>
                    </tr>
                    <tr>
  ...
  <tr>
                        ...
                        <td><select name="d" id="status_registrasi" class="form-control" required>
                            <?php
                                $dt = $this->db->query("select * from statusregistrasi where kodeSR ='$rows[sr]'");
                            $d = $dt->row_array();
                                ?>
                                            <option value="<?=$d['kodeSR']?>"><?=$d['namaSR']?></option>
                                       <?php
                             $bedasal = $this->db->query("select * from statusregistrasi where kodeSR != '$rows[sr]'");
                            foreach($bedasal->result_array() as $bds){
                              ?>
